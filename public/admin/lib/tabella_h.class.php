...
function get_cella($row,$col){
	...
	// Fine Modifica
		...
		case "check1":
			...
			$retval="<td align=\"center\" valign=\"middle\" width=\"7\"><input width=\"7\" type=\"checkbox\" name=\"dati[$group][$nome]\" $strValore $selected></td>\n";
			break;
		case "radio":
		...
		case "color":
			...
			if ($val)
				...
				$retval="<td>Nessun colore definito</td>";
			break;
		case "button":
			$size=explode("#",$w);
			$width=$size[0];
...
function elenco ($message="Nessun dato presente"){
	$ncols=$this->num_col;
	$all="left";
	$tabella="";
	...
	for ($i=0;$i<$ncols;$i++){
		...
		//else
		//if($this->def_col[$i][0])
			$tabella.="\t\t\t\t<td align=\"$all\" width=\"".$this->def_col[$i][2]."\"><font face=\"Verdana\" color=\"$this->color_head_font\" size=\"1\"><b>".$this->def_col[$i][0]."</b></font></td>\n";		
		$all="left";
	}
	$tabella.="\t\t\t</tr>\n";
