...
function navigate(arr_key,arr_val){
	...
	for(i=0;i<arr_key.length;i++){
		...
		f.appendChild(new Element('input',{'type':'hidden','name':name,'value':arr_val[i]}));
	}
	if (arr_key.length>0){
		f.appendChild(new Element('input',{'type':'hidden','name':'livello','value':arr_key[arr_key.length-1]}));
	}
	f.submit();
}
function link(id,level){
	var f = $('frm_param');
	f.appendChild(new Element('input',{'type':'hidden','name':'parametri[]['+level+']','value':id})); // AGGIUNGO AI PARAMETRI IL VALORE DI DESTINAZIONE
	f.appendChild(new Element('input',{'type':'hidden','name':'livello','value':level}));  //LIVELLO DI DESTINAZIONE
	f.submit();
}
...
function showElement(el,i,flag){
	switch (el){
		case "legendtype_id":
			if (flag==2)
				$('upl_legend_icon').disabled=0;
			else
				$('upl_legend_icon').disabled=1;
		case "filetype_id":
			if(flag==-1)
				$('upl_file').disabled=1;
			else
...
function setAction(f,param){
	var frm=$(f);
	if(param) frm.action=frm.action+'?'+param;
}
...
function setControls(arrObj){
	if (is_array(arrObj)){
		for (j=0;j<arrObj.length;j++){
			if(arrObj[j].id){
				var obj=arrObj[j];
				var ctr=($(obj.id))?($(obj.id)):(parent.$(obj.id));
					...
					switch(ctr.type){
						...
						case "select-one":
							for(i=0;i<ctr.options.length;i++){
								if(ctr.options[i].value==obj.value){
