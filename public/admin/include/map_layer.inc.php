	...
	if (count($ris)){
		foreach($ris as $val){
			...
			if($this->mode!=0 || $presente==1)
				$data[]=ARRAY("presente"=>$presente,"layergroup_id"=>$layergroup_id,"layergroup_title"=>$layergroup_title,"theme_title"=>$theme_title,"status"=>$status,"refmap"=>$refmap);
		}
	}
	else{
		$data=Array();
