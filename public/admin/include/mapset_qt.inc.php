...
require_once "../../config/config.php";
	...
	$sql="SELECT DISTINCT coalesce(mapset_name,'') as mapset_name,qt_id,CASE WHEN COALESCE(mapset_name, '') <> '' THEN 1 ELSE 0 END  AS presente,'$project' as project_name,coalesce(qt.qt_name,'') as qt_name,qt_order,coalesce(theme_title,'') as theme_title from (".DB_SCHEMA.".qt inner join ".DB_SCHEMA.".theme using(theme_id)) $JOIN (select * from ".DB_SCHEMA.".mapset_qt where mapset_name='$map') as foo using (qt_id) WHERE project_name='$project' and layer_id in (select distinct layer_id from ".DB_SCHEMA.".mapset_layergroup inner join ".DB_SCHEMA.".layer using(layergroup_id) where mapset_name='$map') order by theme_title,qt_name";
	//echo $sql;
	if($db->sql_query($sql)){
